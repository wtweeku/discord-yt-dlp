...
client.on("messageCreate", async (message) => {
  ...
  if (command === "ping") {
    ...
    const shortVideo = topResults.find(
      (x) => moment(x.duration_raw, "mm:ss").minutes() < 1
    );
    if (shortVideo) {
      return await handleProcess(message, shortVideo.url, true);
    } else {
      return message.reply(
        `No videos under 1 minute found from top 10 results`
      );
    }
...
const handleProcess = async (message, url, reply) => {
  ...
  if ((await getFileSize(filename)) > 8) {
    ...
    if (smallerSize > 8)
      return message.reply(`After downgrade filesize was: ${smallerSize}MiB`);
  } else {
